...
function OverviewPage({ user }) {
  ...
  const todayAtt = store.getAttendance({ date: today });
  const presentToday = new Set(todayAtt.filter((a) => a.type === 'clock-in' && (!isSiteMgr || store.getEmployee(a.employeeId)?.siteId === user.siteId)).map((a) => a.employeeId)).size;
  ...
  const july = '2026-07';
  const payslips = emps.map((e) => store.computePayslip(e.id, july));
  const payrollCost = payslips.reduce((s, p) => s + p.netPay, 0);
  const incentiveTotal = payslips.reduce((s, p) => s + p.incentive, 0);

  // last 7 days attendance bar chart (unique clock-ins per day)
  const days = [];
  for (let d = 9; d <= 15; d++) {
    const key = `2026-07-${String(d).padStart(2, '0')}`;
    const marks = store.getAttendance({ date: key }).filter((a) => a.type === 'clock-in');
    days.push({ label: String(d), value: new Set(marks.map((m) => m.employeeId)).size });
  }

  // Site distribution donut
  const sites = store.getSites();
  const donut = sites.map((s, i) => ({ label: s.city, value: emps.filter((e) => e.siteId === s.id).length, color: ['#1E40AF', '#7C3AED', '#059669', '#EA580C'][i % 4] }));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <StatCard label="Total field staff" value={emps.length} sub={`${sites.length} sites`} icon="users" tone="brand"/>
        ...
        <StatCard label="Payroll (Jul)" value={fmtINR(payrollCost)} sub="Projected net payout" icon="wallet" tone="brand"/>
        <StatCard label="Incentives (Jul)" value={fmtINR(incentiveTotal)} sub="Across all slabs" icon="trending-up" tone="green"/>
      </div>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-12 lg:col-span-2">
          <Card title="Site distribution" bodyClass="p-3">
