...
function ReportsPage({ user }) {
  ...
  // Attendance daily counts July 1..15
  const attSeries = [];
  for (let d = 1; d <= 15; d++) {
    const key = `2026-07-${String(d).padStart(2, '0')}`;
    const marks = store.getAttendance({ date: key }).filter((a) => a.type === 'clock-in');
    attSeries.push({ label: String(d), value: new Set(marks.map((m) => m.employeeId)).size });
  }
  ...
  // Incentive distribution by slab
  const slabs = store.getSlabs();
  const slabDist = slabs.map((s, i) => ({
    label: s.label,
    value: emps.filter((e) => {
      const sales = store.getSales(e.id, july)?.totalSales || 0;
      return store.calcIncentive(sales).slab.id === s.id;
    }).length,
    color: ['#EEF2FF','#C7D2FE','#818CF8','#1E40AF'][i % 4].replace('EEF2FF', '#A5B4FC'),
  }));
  const slabColors = ['#DBEAFE','#93C5FD','#3B82F6','#1E40AF'];
  slabDist.forEach((d, i) => d.color = slabColors[i % slabColors.length]);

  // Site deployment
  const sitesWithCount = store.getSites().map((s) => ({ ...s, staffCount: emps.filter((e) => e.siteId === s.id).length }));
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="col-span-12 lg:col-span-4">
            <Card title="Attendance summary">
              {[
                ...
                ['Perfect attendance', emps.filter((e) => store.computePayslip(e.id, july).absentDays === 0).length + ' employees'],
      ...
      {tab === 'deployment' && (
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-12 lg:col-span-7">
            <Card title="Manpower deployment across India" subtitle="Field staff by client site" bodyClass="p-3">
              <IndiaMapSVG sites={sitesWithCount} height={340}/>
            </Card>
          </div>
          <div className="col-span-12 lg:col-span-5">
            <Card title="Site roster" bodyClass="p-0">
              {sitesWithCount.map((s) => (
